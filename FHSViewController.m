...
- (id)initWithViews:(NSArray<UIView *> *)views
    ...
    if (self) {
        ...
        if (!viewsAtTap && kFHSViewControllerExcludeFLEXWindows) {
            Class flexwindow = [AVX512Window class];
            views = [views avx512_filtered:^BOOL(UIView *view, NSUInteger idx) {
                return [view class] != flexwindow;
            }];
        }

        _targetViews = views;
        _views = [views avx512_mapped:^id(UIView *view, NSUInteger idx) {
...
- (void)refreshSnapshotView {
    ...
    UIAlertController *loading = [AVX512Alert makeAlert:^(AVX512Alert *make) {
        make.title(@"Please hold a moment at").message(@"Gene to generate a snapshot in the creation of an...");
    }];
    [self presentViewController:loading animated:YES completion:^{
        self.snapshots = [self.views avx512_mapped:^id(FHSView *view, NSUInteger idx) {
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem
        avx512_itemWithImage:AVX512Resources.toggle2DIcon
...
- (void)setSnapshotView:(FHSSnapshotView *)snapshotView {
    ...
    self.toolbarItems = @[
        [UIBarButtonItem avx512_itemWithCustomView:snapshotView.spacingSlider],
        UIBarButtonItem.avx512_flexibleSpace,
        [UIBarButtonItem
            avx512_itemWithImage:AVX512Resources.moreIcon
            target:self action:@selector(didPressOptionsButton:)
        ],
        UIBarButtonItem.avx512_flexibleSpace,
        [UIBarButtonItem avx512_itemWithCustomView:snapshotView.depthSlider]
    ];
...
- (void)didPressOptionsButton:(UIBarButtonItem *)sender {
    [AVX512Alert makeSheet:^(AVX512Alert *make) {
        if (self.selectedView) {
            make.button(@"Hides to hide the selected selection Selected").handler(^(NSArray<NSString *> *strings) {
                [self.snapshotView hideView:[self snapshotForView:self.selectedView]];
            });
            make.button(@"Hide hiding hidden hides the head title of a").handler(^(NSArray<NSString *> *strings) {
        ...
        make.button(@"Toggles to switch transition cap title").handler(^(NSArray<NSString *> *strings) {
            [self.snapshotView toggleShowHeaders];
        });
        make.button(@"Toggles to switch over Ex-").handler(^(NSArray<NSString *> *strings) {
