...
@implementation AVX512TableViewSection
...
- (NSArray<UIMenuElement *> *)menuItemsForRow:(NSInteger)row sender:(UIViewController *)sender API_AVAILABLE(ios(13.0)) {
    ...
    if (copyItems.count) {
        ...
        for (NSInteger i = 0; i < copyItems.count; i += 2) {
            ...
            NSString *title = collapseMenu ? key : [@"Copy copy-copy duplicate " stringByAppendingString:key];
        ...
        UIMenu *copyMenu = [UIMenu
            avx512_inlineMenuWithTitle:@"Copy copy-copy duplicate…" 
            image:copyIcon
            children:actions
        ];
        
        if (collapseMenu) {
            return @[[copyMenu avx512_collapsed]];
        } else {
            return @[copyMenu];
        }
    }
