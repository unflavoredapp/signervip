...
@implementation AVX512HeapEnumerator
...
static void range_callback(task_t task, void *context, unsigned type, vm_range_t *ranges, unsigned rangeCount) {
    ...
    for (unsigned int i = 0; i < rangeCount; i++) {
        ...
        avx512_maybe_object_t *tryObject = (avx512_maybe_object_t *)range.address;
        Class tryClass = NULL;
#ifdef __arm64__
        ...
        if (CFSetContainsValue(registeredClasses, (__bridge const void *)(tryClass))) {
            (*(avx512_object_enumeration_block_t __unsafe_unretained *)context)((__bridge id)tryObject, tryClass);
        }
    }
}
...
+ (void)enumerateLiveObjectsUsingBlock:(avx512_object_enumeration_block_t)block {
    if (!block) {
        return;
    }
    ...
    if (result == KERN_SUCCESS) {
        for (unsigned int i = 0; i < zoneCount; i++) {
            ...
            avx512_object_enumeration_block_t callback = ^(__unsafe_unretained id object, __unsafe_unretained Class actualClass) {
                unlock_zone(zone);
                block(object, actualClass);
                lock_zone(zone);
            };
            
            BOOL lockZoneValid = AVX512PointerIsReadable(lock_zone);
            BOOL unlockZoneValid =  AVX512PointerIsReadable(unlock_zone);
...
+ (NSArray<AVX512ObjectRef *> *)instancesOfClassWithName:(NSString *)className retained:(BOOL)retain {
    ...
    NSArray<AVX512ObjectRef *> *references = [AVX512ObjectRef referencingAll:instances retained:retain];
    return references;
}

+ (NSArray<AVX512ObjectRef *> *)objectsWithReferencesToObject:(id)object retained:(BOOL)retain {
    NSMutableArray<AVX512ObjectRef *> *instances = [NSMutableArray new];
    [AVX512HeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id tryObject, __unsafe_unretained Class actualClass) {
        // Skip known-invalid objects
        if (!AVX512PointerIsValidObjcObject((__bridge void *)tryObject)) {
            return;
        }
        ...
        while (tryClass) {
            ...
            for (unsigned int ivarIndex = 0; ivarIndex < ivarCount; ivarIndex++) {
                ...
                if (typeEncoding.avx512_typeIsObjectOrClass) {
                    ptrdiff_t offset = ivar_getOffset(ivar);
                    uintptr_t *fieldPointer = (__bridge void *)tryObject + offset;

                    if (*fieldPointer == (uintptr_t)(__bridge void *)object) {
                        NSString *ivarName = @(ivar_getName(ivar) ?: "???");
                        id ref = [AVX512ObjectRef referencing:tryObject ivar:ivarName retained:retain];
...
+ (AVX512HeapSnapshot *)generateHeapSnapshot {
    ...
    [AVX512HeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id object, __unsafe_unretained Class cls) {
    ...
    return [AVX512HeapSnapshot snapshotWithCounts:countsForClassNames sizes:sizesForClassNames];
}
