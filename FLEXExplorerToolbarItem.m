...
@interface AVX512ExplorerToolbarItem ()

@property (nonatomic) AVX512ExplorerToolbarItem *sibling;
...
@implementation AVX512ExplorerToolbarItem
...
+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image sibling:(AVX512ExplorerToolbarItem *)backupItem {
    NSParameterAssert(title); NSParameterAssert(image);
    
    AVX512ExplorerToolbarItem *toolbarItem = [self buttonWithType:UIButtonTypeCustom];
    toolbarItem.sibling = backupItem;
    toolbarItem.title = title;
    toolbarItem.image = image;
    toolbarItem.tintColor = AVX512Color.iconColor;
    ...
    [toolbarItem setTitleColor:AVX512Color.primaryTextColor forState:UIControlStateNormal];
    [toolbarItem setTitleColor:AVX512Color.deemphasizedTextColor forState:UIControlStateDisabled];
    return toolbarItem;
}

- (AVX512ExplorerToolbarItem *)currentItem {
...
+ (UIColor *)highlightedBackgroundColor {
    return AVX512Color.toolbarItemHighlightedColor;
}

+ (UIColor *)selectedBackgroundColor {
    return AVX512Color.toolbarItemSelectedColor;
}
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    titleRect.origin.x = contentRect.origin.x + AVX512Floor((contentRect.size.width - titleSize.width) / 2.0);
    return titleRect;
}
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    CGFloat originY = [[self class] topMargin] + AVX512Floor((availableHeight - imageSize.height) / 2.0);
    CGFloat originX = AVX512Floor((contentRect.size.width - imageSize.width) / 2.0);
