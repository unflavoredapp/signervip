...
- (id)initWithView:(UIView *)view isInScrollView:(BOOL)inScrollView {
    ...
    if (self) {
        ...
        UIViewController *controller = [AVX512Utility viewControllerForView:view];
        if (controller) {
            _important = YES;
...
- (NSArray<FHSView *> *)children {
    ...
    return [self.view.subviews avx512_mapped:^id(UIView *subview, NSUInteger idx) {
...
+ (UIImage *)_snapshotView:(UIView *)view {
    ...
    [view.subviews avx512_forEach:^(UIView *v, NSUInteger idx) {
        if (!v.isHidden) {
            v.hidden = YES;
