...
@interface AVX512NavigationController ()
...
@implementation AVX512NavigationController

+ (instancetype)withRootViewController:(UIViewController *)rootVC {
    AVX512NavigationController *nav = [[self alloc] initWithRootViewController:rootVC];
    return nav;
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (self.waitingToAddTab) {
        ...
        if ([self.presentingViewController isKindOfClass:[AVX512ExplorerViewController class]]) {
            // New navigation controllers always add themselves as new tabs,
            // tabs are closed by AVX512ExplorerViewController
            [AVX512TabList.sharedList addTab:self];
...
@implementation UINavigationController (AVX512ObjectExploring)
