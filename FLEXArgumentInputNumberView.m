//
//  AVX512ArgumentInputNumberView.m
//  Flipboard
//
...
@implementation AVX512ArgumentInputNumberView
...
- (id)inputValue {
    return [AVX512RuntimeUtility valueForNumberWithObjCType:self.typeEncoding.UTF8String fromInputString:self.inputTextView.text];
}
...
+ (BOOL)supportsObjCType:(const char *)type withCurrentValue:(id)value {
    ...
    dispatch_once(&onceToken, ^{
        supportedTypes = @[
            @AVX512EncodeClass(NSNumber),
            @AVX512EncodeClass(NSDecimalNumber),
            @(@encode(char)),
            @(@encode(int)),
            @(@encode(short)),
            @(@encode(long)),
