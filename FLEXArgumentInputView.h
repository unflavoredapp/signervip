...
typedef NS_ENUM(NSUInteger, AVX512ArgumentInputViewSize) {
    /// 2 lines, medium-sized
    AVX512ArgumentInputViewSizeDefault = 0,
    /// One line
    AVX512ArgumentInputViewSizeSmall,
    /// Several lines
    AVX512ArgumentInputViewSizeLarge
};

@protocol AVX512ArgumentInputViewDelegate;

@interface AVX512ArgumentInputView : UIView
...
@property (nonatomic) AVX512ArgumentInputViewSize targetSize;

/// Users of the input view can get delegate callbacks for incremental changes in user input.
@property (nonatomic, weak) id <AVX512ArgumentInputViewDelegate> delegate;
...
/// Used by AVX512ArgumentInputViewFactory to create appropriate input views.
...
@protocol AVX512ArgumentInputViewDelegate <NSObject>
