...
typedef void (^avx512_object_enumeration_block_t)(__unsafe_unretained id object, __unsafe_unretained Class actualClass);

/// Counts and identifies all class instances on the heap.
@interface AVX512HeapSnapshot : NSObject
...
@interface AVX512HeapEnumerator : NSObject
...
+ (void)enumerateLiveObjectsUsingBlock:(avx512_object_enumeration_block_t)callback
...
/// To validate them yourself, pass each reference's object to \c AVX512PointerIsValidObjcObject
+ (NSArray<AVX512ObjectRef *> *)instancesOfClassWithName:(NSString *)className retained:(BOOL)retain;

/// Returned references have been validated via \c AVX512PointerIsValidObjcObject
